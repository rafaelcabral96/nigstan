...
$$
where $\Lambda_t \sim NIG(\eta^\star,\mu^\star)$ independent from $X_t$. In our framework we have $\mathbf{D}_{AR1}\mathbf{x}=\sigma\mathbf{\Lambda}(\eta^\star,\mu^\star)$, where:
...
\end{pmatrix}, 
$$
where the first element of the matrix is $\sqrt{1-\rho^2}$ so that $x_t$ is a stationary process.

We study the end-of-day closing prices of the Google equity obtained from Yahoo finance from 31 December 2014 to 30 April 2021. This dataset was previously studied in @dhull2021normal who used an autoregressive process of order 1 as suggested by the PACF plot. The previous authors used a conditional representation similar to eq. \@ref(eq:noise0) and the auxiliary variables $V_i$ to obtain MLE estimates for the parameters using the EM algorithm. However, this problem could have been turned into a simple optimization problem by using the joint density in \@ref(eq:joint) where is is possible to relate the data directly with the model parameters without needing auxiliary variables. Since there is a clear positive trend in the data, we also add a drift parameter $c$:
...
The PC prior for $\sigma$ is an exponential distribution with some rate parameter $\theta_\sigma$.  To find this rate parameter we relate $\sigma$ with a more intuitive quantity which is the marginal standard deviation $\sigma_{marg} = \sigma/\sqrt{1-\rho^2}$. As suggested in @simpson2017penalising we use the probability statement $P(\sigma_{marg}>U_\sigma)=\alpha_\sigma$, leading to:

$$
\theta_{\sigma}(\rho) = -\frac{\log(\alpha_\sigma)}{U_\sigma\sqrt{1-\rho^2}}
$$
and so the log-density of the prior for $\sigma$ conditioned on $\rho$ is given by:
...
where we considered $P(\sigma_{marg}>100)=0.1$.
...
We need an intuitive criterion to scale the prior, i.e. find the parameter $\theta$. One option suggested in @PCAR is to find $\theta$ in terms of the probability statement $P(\rho>U_\rho)=\alpha_\rho$. The solution to this equation is given implicitly by:
...
As motivated in ?? we used an exponential prior for $\eta^\star$ and a laplace prior for $\mu^\star$, both with different rate parameters. We scaled the PC priors for $\eta^\star$ and $\mu^star$ based on the following probability statements: $P(Q(\eta^\star)>2)=0.1$ and $P(|\gamma|>2)=0.1$. $Q(\eta^\star)$ quantifies how much more large events (larger than $3\sigma$) the NIG noise has compared with Gaussian noise in the symmetric case ($\mu^\star=0$): $$Q(\eta^\star)=\frac{P(|\Lambda_i|>3\sigma)}{P(|Z_i|>3\sigma)}.$$
The probability statements lead to the rate parameters $\theta_{\eta^\star}\approx 15$ and $\theta_{\mu^\star} \approx 6.5$. These priors are declared in the Stan model as follows:
...
We need to pass to Stan the number of observations, the equity data and the rate parameters of the priors of $\eta^\star$ and $\mu^\star$.
...
```
The fit was successful with no warning messages. The parameter $\rho$ is very close to 1, so the model behaves close to a RW1 model. 
...
```{}
for (n in 2:N)
  x[n] ~ NIG(c + rho*x[n-1], sigma, etas, mus, 1);
```
...
```{r}
fit_NIG <- readRDS("../files/fits/ARNIG.rds")
knitr::kable(head(fit_NIG$summary(), 6), "simple", row.names = NA, digits=2)
```

The fit was successful with no warning messages. The parameter $\eta^\star$ is quite large (posterior mean of 48) indicating a clear heavy tailed behavior for the noise, while the parameter $\mu^\star$ was close to 0, suggesting no need for an asymmetric distribution for the noise.

### Comparizon

We compare the Gaussian and NIG latent models using the leave-one-out cross-validation estimates of @vehtari2017practical, which gave unequivocal preference for the NIG model.
...
NIG.x1594 = c + rho*data[N]     + sigma*rNIG(n_draws, 0, 1, etas, mus, h = 1)
NIG.x1595 = c + rho*NIG.x1594   + sigma*rNIG(n_draws, 0, 1, etas, mus, h = 1)
NIG.x1596 = c + rho*NIG.x1595   + sigma*rNIG(n_draws, 0, 1, etas, mus, h = 1)
```
